...
class SmallMachineSimulator:
    def __init__(self, memory_gb):
        ...
        if self.eat_bytes <= 0:
            print(f"Current total RAM ({self.total_bytes / (1024**3):.2f} GB) is already less than or equal to memory limit ({memory_gb} GB).")
            sys.exit(0)

    def allocate(self):
        print(f"System Total RAM: {self.total_bytes / (1024**3):.2f} GB")
        print(f"Memory limit to simulate: {self.memory_bytes / (1024**3):.2f} GB")
        print(f"Attempting to lock approximately {self.eat_bytes / (1024**3):.2f} GB of RAM into physical memory...")
        ...
        try:
            ...
            print("Allocation complete. Simulation is active.")
            print("Press Ctrl+C to stop the simulation and free memory.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Simulate living with restricted computer resources.")
    parser.add_argument("--memory", type=float, default=8.0, help="Memory limit in GB (default: 8.0)")
    args = parser.parse_args()

    simulator = SmallMachineSimulator(args.memory)
